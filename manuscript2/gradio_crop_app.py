#!/usr/bin/env python3
"""
Gradio UI , curated images:
- Auto-loads correct DL arch (MLP / LSTM / Transformer) from model_meta.json
- Compact left panel: Soil row + Climate row
- Sample table row-click autofill (Gradio v4)
- 2-row Top probabilities (Top-1 & Top-2)
- Robust local image loader (case-insensitive, spaces/underscores ignored)
- Clean LLM markdown recommendations

Run:
  python gradio_crop_app.py --model_dir m2_outputs --images_dir images --ollama_model llama3 --port 7860
"""
...
# Optional torch (only if your best model is DL)
try:
    import torch
...
def load_artifacts(model_dir: Path):
    """
    Expects files created by m2 training:
      - model_type.txt            -> "sklearn" or "dl"
      - classes.json              -> ordered class labels
      - label_encoder.joblib      -> sklearn LabelEncoder
      - (sklearn) best_model.joblib
      - (dl) model_meta.json, best_model.pt, scaler.json
    """
...
# -------------------- Ollama (LLM) --------------------
def ask_ollama(ollama_model: str, crop: str, row: Dict[str, Any], timeout=180) -> str:
    if not ollama_model:
    ...
    try:
        payload = {
            ...
            "messages": [
                {"role": "system", "content":
                 "You are an agronomy assistant. Answer in clean markdown with section headers and short bullets."},
                {"role": "user", "content":
                 (f"Predicted crop: {crop}\n"
                  ...
                  "Write 5–7 concise recommendations using these sections:\n"
                  "### NPK & Timing\n- ...\n"
                  "### pH Management\n- ...\n"
                  "### Irrigation\n- ...\n"
        ...
        if r.status_code == 200:
            return (r.json().get("message", {}) or {}).get("content", "").strip() or "(No content returned)"
...
def on_sample_select(df: pd.DataFrame, evt: gr.SelectData):
    """
    Fill the 7 inputs from the clicked row (order: N,P,K,T,H,pH,R).
    Handles evt/index None (header click, initial renders).
    """
...
def build_interface(model_dir: Path, images_dir: Path, ollama_model: str, port: int):
    ...
    def predict_fn(N, P, K, T, H, pH, R, ask_llm):
        ...
        # 2-row Top-K table
        if proba is None:
            top_df = pd.DataFrame([{"Rank": "Top-1", "Class": crop, "Prob": 1.0},
                                   {"Rank": "Top-2", "Class": "", "Prob": ""}],
                                   columns=["Rank","Class","Prob"])
        else:
            classes = np.array(loaded["classes"])
            order = np.argsort(-proba[0])[:2]
            rows = [{"Rank": f"Top-{i+1}", "Class": classes[idx], "Prob": round(float(proba[0, idx]), 4)}
                    for i, idx in enumerate(order)]
            while len(rows) < 2:
                rows.append({"Rank": f"Top-{len(rows)+1}", "Class": "", "Prob": ""})
            top_df = pd.DataFrame(rows, columns=["Rank","Class","Prob"])

        img = find_crop_image(crop, img_index) or placeholder_image(crop)
        reco_text = ask_ollama(ollama_model, crop, row) if ask_llm else "(Ollama disabled)"
        return crop, top_df, img, reco_text

    # ------------------- Gradio Layout -------------------
    with gr.Blocks(css=CUSTOM_CSS, title="Crop Prediction & Recommendations") as demo:
        gr.HTML("<h1 class='title'>🌾 AI-Powered Crop Prediction & Smart Yield Recommendations</h1>")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_classes=["card"]):
                ...
                ask = gr.Checkbox(label=f"Ask LLM ({ollama_model}) for recommendations", value=True)
                ...
                # Proper v4 select wiring: pass df as input, Gradio appends evt
                ex_table.select(
                    fn=on_sample_select,
                    inputs=[ex_table],
                    outputs=[N, P, K, T, H, pHv, R]
                )
            ...
            with gr.Column(scale=1, elem_classes=["card"]):
                ...
                reco = gr.Markdown(
                    "Toggle the checkbox to get crop-specific tips.",
        ...
        btn.click(
            predict_fn,
            inputs=[N, P, K, T, H, pHv, R, ask],
            outputs=[pred_crop, conf_table, img_out, reco]
        )
